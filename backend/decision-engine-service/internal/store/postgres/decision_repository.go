...
func (r DecisionRepository) ListFilteredCursor(ctx context.Context, tenantID string, filter ports.DecisionListFilter, limit int, cursor *ports.DecisionListCursor) ([]decision.Decision, bool, error) {
	stmt, args := buildFilteredDecisionCursorQuery(filter, cursor)
	return r.listCursor(ctx, stmt, append([]any{tenantID}, append(args, limit)...)...)
}
...
func (r DecisionRepository) list(ctx context.Context, stmt string, args ...any) ([]decision.Decision, error) {
	...
	defer rows.Close()
	return scanDecisionSummaries(rows)
}
...
func (r DecisionRepository) listPage(ctx context.Context, stmt string, args ...any) ([]decision.Decision, bool, error) {
	...
	defer rows.Close()
	items, err := scanDecisionSummaries(rows)
	if err != nil {
		return nil, false, err
	}
	if len(items) == 0 {
...
func (r DecisionRepository) listCursor(ctx context.Context, stmt string, args ...any) ([]decision.Decision, bool, error) {
...
func buildFilteredDecisionQuery(filter ports.DecisionListFilter, paged bool) (string, []any) {
	joinSQL, whereSQL, args := buildDecisionFilterWhereClause(filter, 2)
	stmt := fmt.Sprintf(`
		select d.id, d.tenant_id, d.scenario_id, d.scenario_iteration_id, d.object_id, d.object_type, d.outcome, d.score, d.triggered, d.created_at
...
func buildFilteredDecisionCursorQuery(filter ports.DecisionListFilter, cursor *ports.DecisionListCursor) (string, []any) {
	joinSQL, whereSQL, args := buildDecisionFilterWhereClause(filter, 2)
	cursorClause := ""
	if cursor != nil {
		cursorArg := len(args) + 2
		cursorIDArg := len(args) + 3
		cursorClause = fmt.Sprintf("\n\t\tand (d.created_at < $%d or (d.created_at = $%d and d.id < $%d::uuid))", cursorArg, cursorArg, cursorIDArg)
	...
	stmt := fmt.Sprintf(`
		...
		from core.decisions d
		%s
		where d.tenant_id = $1%s%s
		order by d.created_at desc, d.id desc
		limit $%d
	`, joinSQL, whereSQL, cursorClause, len(args)+2)
	return stmt, args
}
...
func buildDecisionFilterWhereClause(filter ports.DecisionListFilter, nextArg int) (string, string, []any) {
	clauses := make([]string, 0, 7)
	args := make([]any, 0, 7)
	joinSQL := ""
	if filter.DecisionID != "" {
		clauses = append(clauses, fmt.Sprintf("d.id = $%d::uuid", nextArg))
	...
	if filter.ObjectIDPrefix != "" {
		clauses = append(clauses, fmt.Sprintf("d.object_id like $%d", nextArg))
		args = append(args, filter.ObjectIDPrefix+"%")
		nextArg++
	}
	if filter.ObjectType != "" {
		clauses = append(clauses, fmt.Sprintf("d.object_type = $%d", nextArg))
		args = append(args, filter.ObjectType)
		nextArg++
	}
	if filter.ObjectID != "" {
		clauses = append(clauses, fmt.Sprintf("d.object_id = $%d", nextArg))
	...
	if filter.Search != "" {
		joinSQL = "left join core.scenarios s on s.tenant_id = d.tenant_id and s.id = d.scenario_id"
		clauses = append(
			clauses,
			fmt.Sprintf("(d.id::text ilike $%d or d.object_id ilike $%d or d.object_type ilike $%d or coalesce(s.name, '') ilike $%d)", nextArg, nextArg, nextArg, nextArg),
		)
...
func scanDecisionSummaries(rows rowScanner) ([]decision.Decision, error) {
	var items []decision.Decision
	for rows.Next() {
		var item decision.Decision
		var outcome string
