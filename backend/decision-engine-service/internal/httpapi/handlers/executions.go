...
func (h ExecutionHandler) ListScheduledExecutionsByScenario(c *gin.Context) {
	tenantID := c.Param("tenantId")
	scenarioID := c.Param("scenarioId")
	items, err := h.executionService.ListScheduledExecutionsByScenarioFiltered(c.Request.Context(), tenantID, scenarioID, parseExecutionListFilter(c))
	if err != nil {
...
func (h ExecutionHandler) GetScheduledExecutionStatusSummary(c *gin.Context) {
	tenantID := c.Param("tenantId")
	scenarioID := c.Param("scenarioId")
	summary, err := h.executionService.GetScheduledExecutionStatusSummary(c.Request.Context(), tenantID, scenarioID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "scheduled_execution_status_summary_failed", "details": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"status_summary": dto.AdaptExecutionStatusSummary(summary)})
}
...
func (h ExecutionHandler) RetryScheduledExecution(c *gin.Context) {
	tenantID := c.Param("tenantId")
	scenarioID := c.Param("scenarioId")
	executionID := c.Param("executionId")
	item, err := h.executionService.RetryScheduledExecution(c.Request.Context(), tenantID, scenarioID, executionID)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "retry_scheduled_execution_failed", "details": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"scheduled_execution": dto.AdaptScheduledExecution(item)})
}
...
func (h ExecutionHandler) ListAsyncDecisionExecutionsByTenant(c *gin.Context) {
	tenantID := c.Param("tenantId")
	items, err := h.executionService.ListAsyncDecisionExecutionsByTenantFiltered(c.Request.Context(), tenantID, parseExecutionListFilter(c))
	if err != nil {
...
func (h ExecutionHandler) GetAsyncDecisionExecutionStatusSummary(c *gin.Context) {
	tenantID := c.Param("tenantId")
	summary, err := h.executionService.GetAsyncDecisionExecutionStatusSummary(c.Request.Context(), tenantID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "async_decision_execution_status_summary_failed", "details": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"status_summary": dto.AdaptExecutionStatusSummary(summary)})
}

func (h ExecutionHandler) GetAsyncDecisionExecution(c *gin.Context) {
	tenantID := c.Param("tenantId")
	executionID := c.Param("executionId")
	item, err := h.executionService.GetAsyncDecisionExecutionByID(c.Request.Context(), tenantID, executionID)
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "async_decision_execution_not_found", "details": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"async_decision_execution": dto.AdaptAsyncDecisionExecution(item)})
}

func (h ExecutionHandler) RetryAsyncDecisionExecution(c *gin.Context) {
	tenantID := c.Param("tenantId")
	executionID := c.Param("executionId")
	item, err := h.executionService.RetryAsyncDecisionExecution(c.Request.Context(), tenantID, executionID)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "retry_async_decision_execution_failed", "details": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"async_decision_execution": dto.AdaptAsyncDecisionExecution(item)})
}
...
func parseExecutionListFilter(c *gin.Context) service.ExecutionListFilter {
	filter := service.ExecutionListFilter{}
	if status := c.Query("status"); status != "" {
		filter.Status = execution.Status(status)
	}
	if minAttempts := c.Query("min_attempt_count"); minAttempts != "" {
		if parsed, err := strconv.Atoi(minAttempts); err == nil && parsed > 0 {
			filter.MinAttemptCount = parsed
		}
	}
