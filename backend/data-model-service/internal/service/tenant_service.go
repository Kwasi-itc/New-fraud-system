...
func (s TenantService) Create(ctx context.Context, input tenant.CreateInput) (tenant.Tenant, error) {
	...
	if err := s.txManager.Run(ctx, func(store ports.MutationStore) error {
		...
		recordTenantSchemaMigration(ctx, store.TenantSchemaMigrations(), s.idGenerator, record.ID, schemaMigrationVersion("create_tenant", "tenant"), record.CreatedAt)
		return nil
	}); err != nil {
		return tenant.Tenant{}, err
	}
...
func (s TenantService) Provision(ctx context.Context, id uuid.UUID) (tenant.Tenant, error) {
	...
	if err := s.txManager.Run(ctx, func(store ports.MutationStore) error {
		...
		recordTenantSchemaMigration(ctx, store.TenantSchemaMigrations(), s.idGenerator, record.ID, schemaMigrationVersion("provision_tenant_schema", "tenant"), now)
