...
BASE_URL="${PRODUCTION_REPLAY_BASE_URL:-${BASE_URL:-}}"
...
if [[ -z "$DATA_MODEL_URL" || -z "$INGESTION_URL" || -z "$DECISION_ENGINE_URL" ]]; then
  printf 'error: set BASE_URL or explicitly set DATA_MODEL_URL, INGESTION_URL, and DECISION_ENGINE_URL for remote replay\n' >&2
  exit 1
fi
...
(
  ...
  PYTHONPATH=stress-tests "$VENV_DIR/bin/python" -u -m production_replay run "${RUN_ARGS[@]}"
) | tee "$REPLAY_LOG"
