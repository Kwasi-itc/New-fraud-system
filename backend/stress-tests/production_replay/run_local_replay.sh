#!/usr/bin/env bash
  ...
  DATA_MODEL_FORWARDED_PORT="${DATA_MODEL_FORWARDED_PORT:-8080}"
  INGESTION_FORWARDED_PORT="${INGESTION_FORWARDED_PORT:-8081}"
  DECISION_ENGINE_FORWARDED_PORT="${DECISION_ENGINE_FORWARDED_PORT:-8082}"
  FRONTEND_FORWARDED_PORT="${FRONTEND_FORWARDED_PORT:-3000}"

  # Explicit URLs take precedence. Otherwise, use the configured forwarded ports.
  DATA_MODEL_URL="${PRODUCTION_REPLAY_DATA_MODEL_URL:-${DATA_MODEL_URL:-http://127.0.0.1:${DATA_MODEL_FORWARDED_PORT}}}"
  INGESTION_URL="${PRODUCTION_REPLAY_INGESTION_URL:-${INGESTION_URL:-http://127.0.0.1:${INGESTION_FORWARDED_PORT}}}"
  DECISION_ENGINE_URL="${PRODUCTION_REPLAY_DECISION_ENGINE_URL:-${DECISION_ENGINE_URL:-http://127.0.0.1:${DECISION_ENGINE_FORWARDED_PORT}}}"
  ...
  require_positive_port "DATA_MODEL_FORWARDED_PORT" "$DATA_MODEL_FORWARDED_PORT"
  require_positive_port "INGESTION_FORWARDED_PORT" "$INGESTION_FORWARDED_PORT"
  require_positive_port "DECISION_ENGINE_FORWARDED_PORT" "$DECISION_ENGINE_FORWARDED_PORT"
  require_positive_port "FRONTEND_FORWARDED_PORT" "$FRONTEND_FORWARDED_PORT"
  require_positive_port "ASYNC_CALLBACK_PORT" "$ASYNC_CALLBACK_PORT"
  ...
  if [[ "$DECISION_MODE" == "async" && -z "$ASYNC_CALLBACK_URL" ]]; then
    ...
    PYTHONPATH="$BACKEND_DIR/stress-tests" \
      "$VENV_DIR/bin/python" -m production_replay.callback_server \
        --host 0.0.0.0 \
    ...
    wait_for_service \
      ...
      "http://127.0.0.1:$ASYNC_CALLBACK_PORT/readyz"
  ...
  (
    ...
    PYTHONPATH=stress-tests \
      "$VENV_DIR/bin/python" \
      -m production_replay.local_sample \
      "${SAMPLE_ARGS[@]}"
  )

  printf 'Preparing the local replay tenant and loading reference data...\n'
  ...
  export DATA_MODEL_FORWARDED_PORT
  export INGESTION_FORWARDED_PORT
  export DECISION_ENGINE_FORWARDED_PORT
  export FRONTEND_FORWARDED_PORT
  ...
  if [[ "$DECISION_MODE" == "async" && "$AUTO_CALLBACK_SERVER" == "1" ]]; then
    ...
    PYTHONPATH="$BACKEND_DIR/stress-tests" \
      "$VENV_DIR/bin/python" \
      -m production_replay.callback_report \
